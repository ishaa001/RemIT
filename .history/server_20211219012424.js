...
app.post('/accepted_req',async(req,res) => {
    ...
    console.log("Got project: " + req.body.group + " and user who accepted the request: " + req.body.friend.username)
...
app.post('/invite_members', auth, async (req, res) => {
    try {
        ...
        console.log("Got findUser: " + findUser.email)

        if(findUser != null){
            // res.status(200).send("Invitation send to " + findUser.email);
            console.log("From: " + req.user.email + " and to: " + findUser.email)
