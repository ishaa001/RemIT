...
app.get('/fetch_task', auth,async (req,res) => {
    try{
        var tks = await tasks.find({usr:req.user._id});
        //     ,function(err,data){
        //     if(data){
        ...
        for(var i = 0;i < tks.length;i++){
            let pjname = tks[i].projname;
            let pjs = await projs.find({name:pjname});
            console.log("\n I got projects - " + pjs + "\n");
            projects.push(pjs);  
        }
        console.log("got tasks - " + tks + " and projs - " + projects);
        var flags = [], output = [];
        console.log("Length is " + projects.length + " and 0th proj is " + projects[0].name);
        for(var i = 0;i < projects.length;i++){
            if(flags[projects[i].name]){
                continue;
            }
            flags[projects[i].name] = true;
            console.log("Got proj " + projects[i].name + " and pushing it " );
            output.push(projects[i].name);
        }
        console.log("Final list of projects is - " + output);
        res.render("tasks",{
            'user' : req.user,
            'tasks' : tks,
            'projects' : output
