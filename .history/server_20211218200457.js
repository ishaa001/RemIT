...
function inviteThroughMail(from,mail,group)
{
    var mailOptions = {
        ...
        html:'<p> In order to be a member of a group, if you want to accept then click on accept button or else ignore this mail.</p><br><form action="/accepted_req" method="POST"><input type="hidden" value="' + group + '" name="group"><input type="hidden" value="' + mail + '" name="friend"><input type="submit" value="Accept"></form>'
    }
...
app.post('/accepted_req',async(req,res) => {
    
        console.log("Reached to accept request!!");
        console.log("Got project: " + req.body.group + " and user who accepted the request: " + req.body.friend)
        res.send("Congratulations!! You are now part of " + req.body.group);
...
app.post('/invite_members', auth, async (req, res) => {
    try {
        ...
        if(findUser != null){
            ...
            inviteThroughMail(req.user.email,findUser.email,req.body.name)
