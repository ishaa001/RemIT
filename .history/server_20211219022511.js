...
function inviteThroughMail(from,mail,group)
{
    ...
    var mailOptions = {
        ...
        html:'<p> In order to be a member of a group, if you want to accept then click on accept button or else ignore this mail.</p><br><form action="http://localhost:3000/accepted_req" method="POST"><input type="hidden" value="' + group + '" name="group"><input type="hidden" value="' + mail.email + '" name="friend"><input type="submit" value="Accept"></form>'
    }
...
app.post('/login', async (req, res) => {
    try {
        ...
        if (isPwdMatch) {
            ...
            alert("Congratulations!! You are now a member of group " + req.body.from_mail);
            res.redirect("fetch_task");
        }
        else {
