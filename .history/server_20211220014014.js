...
app.get('/del_projects/:id', auth, async(req, res) => {
...
app.get('/fetch_project_task/:name', auth, async (req, res) => {
    try {
        ...
        for(var i = 0;i < projects.length;i++){
            ...
            // mongoose.mongo.BSONPure.ObjectID
            console.log("---------------- type of user id " + typeof(userID));
            var o_id = mongoose.Types.ObjectId(userID)
            const exists = await user.findOne({"_id":mongoose.Types.ObjectId('618a5bccdccd25497c334844')},function(data,err){
                if(data){
                    // console.log("User data: -------------" + data);
                    console.log("---------------Got data --------------" + data)
                }
                else{
                    console.log("-------------Got error and userID: " + userID)
                    // console.log("Error:( --------" + err + " --------------- and userId ------------" + userID);
                }
            });
            var uname = user.find({_id:userID});
            // console.log(uname)
            user_nms.push(uname.username);
        }
